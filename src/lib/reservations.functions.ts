...
function guestHtml(data: z.infer<typeof schema>) {
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;margin:auto;background:#161412;border:1px solid #2a2622;border-radius:12px;overflow:hidden">
    ...
    <tr><td style="padding:24px 32px">
      <h1 style="font-family:Georgia,serif;font-size:22px;margin:0 0 12px;color:#fff">Hi ${escapeHtml(data.name)},</h1>
      ...
      <table cellpadding="6" style="width:100%;border-collapse:collapse;border-top:1px solid #2a2622;border-bottom:1px solid #2a2622;margin:12px 0">
        ...
        <tr><td style="color:#9b8f7a;font-size:12px;text-transform:uppercase;letter-spacing:.18em">Time</td><td style="text-align:right">${escapeHtml(data.reservation_time)}</td></tr>
      ...
      ${data.requests ? `<p style="color:#cfc7b5;font-size:14px;margin:16px 0 0"><strong style="color:#d4af37">Notes:</strong> ${escapeHtml(data.requests)}</p>` : ""}
...
function adminHtml(data: z.infer<typeof schema>) {
  ...
  <table cellpadding="6" style="border-collapse:collapse">
    ...
    <tr><td><b>Time</b></td><td>${escapeHtml(data.reservation_time)}</td></tr>
    <tr><td><b>Guests</b></td><td>${data.party_size}</td></tr>
    <tr><td><b>Notes</b></td><td>${data.requests ? escapeHtml(data.requests) : "—"}</td></tr>
  </table></body></html>`;
}
